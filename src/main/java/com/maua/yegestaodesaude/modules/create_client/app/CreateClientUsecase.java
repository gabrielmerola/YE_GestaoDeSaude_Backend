...
public class CreateClientUsecase {
    ...
    public ResponseEntity<Object> execute(ClientDto clientDto){
        ...
        if(clientExists != null){
            return ResponseEntity.status(HttpStatusCode.valueOf(409)).body("Client already exists");
        }

        var passwordCrypted = passwordEncoder.encode(clientDto.password());

        Client entity = new Client(clientDto.name(), clientDto.email(), passwordCrypted, clientDto.phone());
        Client newClient = clientRepository.save(entity);
        
        return ResponseEntity.status(HttpStatusCode.valueOf(201)).body(new ClientDto(newClient.getName(), newClient.getEmail(), newClient.getPassword(), newClient.getPhone()));
    }
}
